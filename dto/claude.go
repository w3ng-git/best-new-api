...
// OverrideClaudeCacheTTL walks through a ClaudeRequest and overrides the TTL
// of all existing cache_control blocks to the specified value (e.g. "1h").
func OverrideClaudeCacheTTL(request *ClaudeRequest, ttl string) {
	if ttl == "" || request == nil {
		return
	}
	...
	data, err := common.Marshal(cacheControlBlock{Type: "ephemeral", TTL: ttl})
	if err != nil {
		return
	}
	newCacheControl := json.RawMessage(data)

	// Override system messages
	systemMessages := request.ParseSystem()
	if len(systemMessages) > 0 {
		modified := false
		for i, msg := range systemMessages {
			if len(msg.CacheControl) > 0 {
				systemMessages[i].CacheControl = newCacheControl
				modified = true
			}
		}
		if modified {
	...
	for i, message := range request.Messages {
		...
		for j, block := range content {
			if len(block.CacheControl) > 0 {
				content[j].CacheControl = newCacheControl
				modified = true
			}
		}
		if modified {
			request.Messages[i].SetContent(content)
		}
	}
}
